...
"#;
        ...
        assert_eq!(StatusProvider::hint(&ai_datum), "OpenAI GPT models via official API");
        assert_eq!(DatumProvider::datum_type(&ai_datum), DatumType::Ai);
    }
...
"#;
        ...
        assert_eq!(DatumChecker::version_status(&ai_datum), VersionStatus::Missing);
        assert!(FilterLogic::is_disabled(&ai_datum));
    }
    ...
    fn test_ai_constraint_evaluation_with_env() {
        ...
        unsafe { env::set_var("TEST_AI_KEY", "test-value"); }
...
"#;
        ...
        assert!(DatumChecker::is_installed(&ai_datum));
        assert_eq!(DatumChecker::version_status(&ai_datum), VersionStatus::Unknown);
        assert!(!FilterLogic::is_disabled(&ai_datum));
        
        // Clean up
        unsafe { env::remove_var("TEST_AI_KEY"); }
    }
...
"#;
        ...
        let tools = get_ai_tools_status(path).unwrap();
        ...
        let tools_with_env = get_ai_tools_status(path).unwrap();
        
        // Should include at least the OpenAI provider now
        let openai_tools: Vec<_> = tools_with_env.iter()
