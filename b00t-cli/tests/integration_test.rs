...
fn test_mcp_add_command_mode() {
    ...
    let output = Command::new("cargo")
        .args(&["run", "--bin", "b00t-cli", "--", 
               "--path", path,
               "mcp", "add", "test-server", "--hint", "Test server", 
               "--", "npx", "-y", "@test/server"])
        .output()
...
fn test_mcp_whitelist_rejection() {
    ...
    assert!(stderr.contains("not whitelisted"), "Should reject non-whitelisted command");
}
...
fn test_environment_variable_path_override() {
    ...
    assert!(config_path.exists(), "Config file was not created with env path");
}