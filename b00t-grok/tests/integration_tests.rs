...
use b00t_grok::{BasicChunker, Chunker, GrokClient, GrokError};
use std::env;
...
async fn test_full_workflow_with_real_database() {
    ...
    let digest_result = client
        .digest(TEST_TOPIC, "Additional test content for digest")
        .await;
    ...
    match ask_result {
        Ok(results) => {
            ...
            if !results.is_empty() {
                ...
                println!(
                    "🎯 Best match: {} chars, topic: {}",
                    best_match.content.len(),
                    best_match.metadata.topic
                );

                // Verify result structure
                assert!(!best_match.content.is_empty());
                assert!(
                    best_match.content.to_lowercase().contains("rust")
...
async fn test_connection_error_handling() {
    ...
    let mut bad_client = GrokClient::new(
        "http://192.168.255.255:9999".to_string(),
        "test".to_string(),
    );
    ...
    if let Err(e) = result {
        ...
        println!(
            "✅ Connection error properly categorized: {}",
            e.user_message()
        );
    }
    ...
    if let Err(e) = embedding_result {
        ...
        println!(
            "✅ Environment variable error properly handled: {}",
            e.user_message()
        );
    }
    ...
    #[cfg(feature = "pyo3")]
    {
        ...
        match semantic_result {
            ...
            Err(e) => {
                ...
                println!(
                    "⚠️ Semantic chunker failed (expected if chonkie not installed): {}",
                    e.user_message()
                );
...
async fn test_performance_limits() {
    ...
    match learn_result {
        Ok(chunks) => {
            ...
            for (i, chunk) in chunks.iter().enumerate() {
                ...
                assert!(
                    !chunk.id.to_string().is_empty(),
                    "Chunk {} should have valid UUID",
                    i
                );
            }
        }
    ...
    match query_result {
        Ok(results) => {
            println!(
                "✅ Query completed in {:?}, returned {} results",
                query_duration,
                results.len()
            );
        }
...
fn test_error_categorization_and_recovery() {
    ...
    for (error, expected_category, expected_recoverable) in errors {
        ...
        assert!(
            !user_msg.contains("Error"),
            "User messages should not contain 'Error'"
        );

        println!(
            "✅ {} error properly categorized: {}",
            expected_category, user_msg
        );
    }
